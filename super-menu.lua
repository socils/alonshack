-- =============================================================================
-- [SUPER CORE]: MOTOR NEXUS MULTI-ROLES v6.0 - PREMIUM DARK TECH
...
local Diccionario = {
    ES = {
        ...
        VolverRoles = "< ROLES",
        VolverAtras = "< VOLVER",
        ModoEdicionActivo = "MODO EDICIÓN: Arrastra los botones. Haz click en el Core para guardar.",
...
local CoreConfig = {
    ...
    RolesRegistrados = {},
    ModoDispositivo = "PC", -- "PC" o "MOVIL"
    ModoEdicionHUD = false
}
...
-- =============================================================================
-- ANIMACIÓN DE CARGA AGRADABLE (PREMIUM LOADING)
-- =============================================================================
local function SecuenciaDeCarga()
    ...
    TextoCarga.Font = Enum.Font.Goblin -- Un estilo más premium/tecnológico moderno
    TextoCarga.Text = T("Cargando")
    TextoCarga.TextColor3 = CoreConfig.ColorTema
    TextoCarga.TextSize = 16
    ...
    local EsquinaBarra = Instance.new("UICorner")
    EsquinaBarra.CornerRadius = UDim.new(1, 0)
    EsquinaBarra.Parent = BarraFondo
    local EsquinaProgreso = EsquinaBarra:Clone()
    EsquinaProgreso.Parent = BarraProgreso

    TweenService:Create(BarraProgreso, TweenInfo.new(0.8, Enum.EasingStyle.QuadOut), {Size = UDim2.new(1, 0, 1, 0)}):Play()
    task.wait(0.9)
...
MainCorner.CornerRadius = UDim.new(0, 12) -- Bordes más curvos y amigables
...
-- =============================================================================
-- CAPAS DE NAVEGACIÓN INTERNA (SÚPER MENÚ TRIPLE)
-- =============================================================================
...
-- =============================================================================
-- PANEL DE CONFIGURACIÓN DEL HUD (PC / MÓVIL / EDICIÓN DE BOTONES)
-- =============================================================================
...
-- =============================================================================
-- LOGICA ARRASTRE Y SOPORTE DE EDICIÓN HUD (DRAG & DROP COMPLETO)
-- =============================================================================
...
-- =============================================================================
-- GESTIÓN DE IDIOMAS DINÁMICA (RE-RENDERIZADO DE INTERFAZ)
-- =============================================================================
...
-- =============================================================================
-- SISTEMA INTERACTIVO DE NAVEGACIÓN Y CONFIGURACIÓN DISPOSITIVO
-- =============================================================================
DispositivoBtn.MouseButton1Click:Connect(function()
    ...
    -- Notificar de inmediato a todos los botones del HUD sobre la mutación visual
...
local function AbrirMenuDelRol(datosRol)
    ...
    if datosRol.Habilidades then
        datosRol.Habilidades(RoleContentScroll, function(nombreHabilidad, callbackConfig, dataHabilidadExtra)
            ...
            -- Inyección de botón HUD Reubicable si la habilidad lo pide
            if dataHabilidadExtra and dataHabilidadExtra.RequiereBotonHUD then
                ...
                -- Distribución por defecto escalonada
                ...
                -- Atajo visual para computadoras
